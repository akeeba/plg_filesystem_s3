 ...
 *
 * Tests use this to *observe* — did the import actually change the stored Options? — and to seed
...
class Database
{
	...
	public function __construct(?Configuration $config = null)
	{
		...
		$dsn = sprintf(
			...
			(int) $config->get('db.port', 33319),
			$config->get('db.name', 's3fse2e')
		);
		...
		try
		{
			$this->pdo = new PDO(
				$dsn,
				(string) $config->get('db.user', 's3fse2e'),
				(string) $config->get('db.pass', 's3fse2e'),
				[
