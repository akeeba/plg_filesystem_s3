#!/usr/bin/env bash
#
# One-shot end-to-end test harness for plg_filesystem_s3.
#
# Provisions a throwaway Joomla site inside Docker (Apache + PHP-FPM + MySQL +
# MinIO, all versions configurable) and runs the PHPUnit end-to-end suite against it over
# real HTTP. Every run starts from a completely clean slate, so a suite that fails
# or crashes mid-way never leaves a site behind in an unknown state.
#
...
#   3. Build and bring up the Dockerized stack; create the MinIO bucket and grant
#      the host anonymous read-only access to it.
#   4. Install Joomla using its installation/joomla.php CLI helper.
#   5. Normalise the site configuration (SEF, caching, debug and mail off).
#   6. Build the package with `phing git` and install plg_filesystem_s3.
#   7. Provision the fixtures: the plugin's connections (one per S3 flavour under
#      test), a restricted back-end user, and the seed objects in the bucket.
...
INSTALLER_SCRIPT="${REPO_ROOT}/plugins/filesystem/s3/script.plg_filesystem_s3.php"
INSTALLER_SCRIPT_REL="plugins/filesystem/s3/script.plg_filesystem_s3.php"

# The package is copied into the web root under this name so the in-container installer can reach it.
PACKAGE_IN_SITE="e2e-package.zip"
...
while [ $# -gt 0 ]; do
	case "$1" in
		...
		-h|--help)       sed -n "2,33p" "${SCRIPT_PATH}" | sed 's/^# \{0,1\}//'; exit 0;;
...
# `mc` has no `up`; it runs one-shot, with the full credentials (see docker-compose.yml).
mc_cli()    { DC run --rm -T mc "$@"; }
mc_sh()     { DC run --rm -T --entrypoint bash mc -c "$1"; }
...
if [ "${RUN_MATRIX}" -eq 1 ]; then
	...
	for entry in ${JOOMLA_MATRIX}; do
		...
		for pver in ${phps//,/ }; do
			...
			# Build once, on the first iteration only; the package does not change between Joomla
...
scrub() {
	...
	rm -rf "${SCRIPT_DIR}/seed" "${SCRIPT_DIR}/bulk"
	mkdir -p "${WWW_DIR}" "${SCRIPT_DIR}/seed" "${SCRIPT_DIR}/bulk"
	ok "Clean slate"
}
...
# Reject a PHP version outside what the plugin claims to support, before we spend minutes on it.
...
assert_php_in_range() {
	...
	if version_lt "${n}" "$(normalize_version "${MIN_PHP_VERSION}")"; then
		die "PHP $1 is below the minimum this plugin supports (${MIN_PHP_VERSION}, from ${INSTALLER_SCRIPT_REL})."
	fi

	if ! version_lt "${n}" "$(normalize_version "${MAX_PHP_VERSION}")"; then
		die "PHP $1 is at or above the exclusive maximum this plugin supports (${MAX_PHP_VERSION}, from ${INSTALLER_SCRIPT_REL})."
	fi
}

# Joomla enforces its own PHP floor and refuses to install below it, so a pair like 6.1 on PHP 8.1
# would fail for a reason that has nothing to do with this plugin. Read that floor out of the
...
precheck_requested_version() {
	...
	local range="Joomla ${MIN_JOOMLA_VERSION} or later, and lower than ${MAX_JOOMLA_VERSION} (${INSTALLER_SCRIPT_REL})"

	if [ "${major}" -lt "${minMajor}" ] || [ "${major}" -gt "${maxMajor}" ]; then
		die "Joomla ${v} is not supported: this plugin requires ${range}."
	fi
...
assert_supported_version() {
	if version_lt "$1" "${MIN_JOOMLA_VERSION}" || ! version_lt "$1" "$(normalize_version "${MAX_JOOMLA_VERSION}")"; then
		die "Joomla $1 is not supported: this plugin requires Joomla ${MIN_JOOMLA_VERSION} or later, and lower than ${MAX_JOOMLA_VERSION}."
	fi
}
...
# ---------------------------------------------------------------------------
bring_up_stack() {
	log "Building and starting the stack (PHP ${PHP_VERSION}, Apache ${APACHE_VERSION}, MySQL ${MYSQL_VERSION}, MinIO ${MINIO_VERSION})"
	# The mc service mounts ./seed and ./bulk; it must exist before compose creates the bind mount as root.
	mkdir -p "${SCRIPT_DIR}/seed" "${SCRIPT_DIR}/bulk"
	DC build >/dev/null
	DC up -d
	...
	log "Creating the '${S3_BUCKET}' bucket and granting the host anonymous read-only access"
	# The policy template names the bucket as __BUCKET__. The mc image has bash but no sed.
	tries=0
	until mc_sh "mc mb --ignore-existing 'e2e/${S3_BUCKET}' >/dev/null \
		&& p=\"\$(cat /config/minio-anonymous-policy.json.tpl)\" \
		&& printf '%s' \"\${p//__BUCKET__/${S3_BUCKET}}\" > /tmp/policy.json \
		&& mc anonymous set-json /tmp/policy.json 'e2e/${S3_BUCKET}' >/dev/null" 2>/dev/null; do
		tries=$((tries + 1))
		[ "${tries}" -gt 30 ] && die "MinIO did not become ready, or the bucket could not be created."
		sleep 2
	done

	# Prove the policy took: anonymous listing must work, anonymous writing must not.
	curl -fsS -o /dev/null "http://localhost:${S3_PORT}/${S3_BUCKET}?list-type=2" \
		|| die "Anonymous listing of the bucket failed; the MinIO policy was not applied."
	[ "$(curl -s -o /dev/null -w '%{http_code}' -X PUT --data 'x' "http://localhost:${S3_PORT}/${S3_BUCKET}/anonymous-write-probe.txt")" = "403" ] \
		|| die "Anonymous PUT into the bucket was not refused; the MinIO policy is too broad."
	ok "MinIO is up at http://localhost:${S3_PORT}/${S3_BUCKET} (anonymous, read-only)"
}
...
# written configuration.php, which has varied across the supported range.
# Patching is version-proof.
#
#   set_config <name> <PHP literal>      e.g. set_config caching "0"
set_config() {
...
configure_site() {
	log "Normalising the site configuration"

	# The plugin sends no mail, and nothing in this harness should try to. There is no mail sink
	# in the stack, so switch the mailer off rather than let a stray send hang on a dead SMTP host.
	set_config mailonline "false"

	# SEF off. Tests address the Media Manager as index.php?option=com_media&task=api.files&…, and
	# asserting on a raw Location header is far clearer when Joomla has not rewritten the URL.
	set_config sef         "false"
	set_config sef_rewrite "false"
	set_config sef_suffix  "false"

	# Deterministic, silent, and fast. Joomla's own caching is OFF. The plugin's S3 response cache
	# does not depend on it — it is switched on per connection — and CachingTest uses exactly that.
	...
	# raised by the plugin could never reach the log for a test to find.
	...
	mysql_cli "${DB_NAME}" <<SQL || die "Could not disable the debug plugin."
UPDATE \`${DB_PREFIX}extensions\` SET enabled = 0 WHERE element = 'debug' AND type = 'plugin';
SQL
}
...
build_package() {
	...
	command -v phing >/dev/null 2>&1 || die "phing is not on PATH (needed to build the package). Use --skip-build to reuse an existing package."
	log "Building the package with 'phing git'"
...
pick_package() {
	local pkg
	pkg="$(ls -t "${RELEASE_DIR}"/plg_filesystem_s3-*.zip 2>/dev/null | head -1 || true)"
...
install_package() {
	...
	cp "${pkg}" "${WWW_DIR}/${PACKAGE_IN_SITE}"
	php_cli cli/joomla.php extension:install --path="/var/www/html/${PACKAGE_IN_SITE}" \
		|| die "Package installation failed."
	# The installer reports some refusals (e.g. an unsupported Joomla or PHP version) as a warning
	# while still exiting 0. Confirm the plugin really is registered before going any further.
	[ -n "$(mysql_cli -N "${DB_NAME}" -e "SELECT extension_id FROM \`${DB_PREFIX}extensions\` WHERE element = 's3' AND folder = 'filesystem' AND type = 'plugin'")" ] \
		|| die "The installer exited cleanly, but plg_filesystem_s3 is not installed."
	# The package is not needed in the web root any more.
	rm -f "${WWW_DIR}/${PACKAGE_IN_SITE}"
...
return [
	...
	's3'      => [
		...
		'publicUrl'        => 'http://localhost:${S3_PORT}',
...
provision_fixtures() {
	log "Provisioning the plugin's connections, the restricted user and the bucket's seed objects"
...
# Main
# ---------------------------------------------------------------------------
log "plg_filesystem_s3 end-to-end harness — Joomla ${JOOMLA_VERSION} on PHP ${PHP_VERSION}"
...
if [ "${KEEP_CONTAINERS}" -eq 1 ] || [ "${RUN_TESTS}" -eq 0 ]; then
	...
	echo "     Site:   http://localhost:${HTTP_PORT}  (admin / ${ADMIN_PASSWORD})"
	echo "     Bucket: http://localhost:${S3_PORT}/${S3_BUCKET}?list-type=2  (anonymous, read-only)"
